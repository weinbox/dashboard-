...
// Base path the dashboard is served from (Supabase edge functions live under /functions/v1).
const BASE = "/functions/v1/admin";
...
function layout(active: string, body: string): string {
  const nav = [
    { href: `${BASE}`, label: "نظرة عامة", key: "overview" },
    { href: `${BASE}/orders`, label: "الطلبات", key: "orders" },
    { href: `${BASE}/pricing`, label: "الأسعار", key: "pricing" },
    { href: `${BASE}/users`, label: "المستخدمون", key: "users" },
  ];
  return `<!doctype html>
...
<div class="wrap">
  <aside>
    ...
    <a class="logout" href="${BASE}/logout">تسجيل الخروج</a>
  </aside>
  <main>${body}</main>
</div>
</body>
...
function notConfiguredPage(): string {
  return layout(
    ...
    `<h1>اللوحة غير مهيّأة</h1>
     <div class="empty">
       لتشغيل اللوحة عيّن السر التالي للدالة ثم أعد النشر:<br/><br/>
       <code>ADMIN_PASSWORD</code><br/><br/>
       (مفتاح <code>SUPABASE_SERVICE_ROLE_KEY</code> متوفر تلقائياً في Supabase)
     </div>`
  );
}
...
async function ordersPage(active: string): Promise<string> {
  const sb = getAdmin();
  let q = sb
    .from("orders")
    .select("id, customer_phone, status, total_items, note, created_at, order_items(*)")
  ...
  const tabsHtml = tabs
    .map(
      (t) =>
        `<a class="${active === t.value ? "active" : ""}" href="${BASE}/orders${t.value === "all" ? "" : `?status=${t.value}`}">${t.label}</a>`
    )
    .join("");
        ...
        .map((o: any) => {
          ...
          const items = (o.order_items ?? [])
            ...
            .map(
              ...
              (it: any) => `
              <div class="item">
                ${it.image ? `<img src="${esc(it.image)}" alt="" />` : `<div style="width:60px;height:60px;border-radius:8px;background:#e5e7eb;display:flex;align-items:center;justify-content:center">🛍</div>`}
                <div style="flex:1">
                  ...
                  <div style="margin-top:4px;font-size:13px">
                    ...
                    <span class="muted">×${esc(it.quantity)}</span>
          ...
          <div class="order">
            <div class="top">
              <div>
                ...
                <span class="muted">#${esc(String(o.id).slice(0, 8))}</span>
                <div style="font-weight:600;margin-top:6px">${esc(o.customer_phone ?? "زبون غير مسجّل")}</div>
                <div class="muted">${fmtDate(o.created_at)}</div>
              </div>
              <form method="post" action="${BASE}/orders/status" style="display:flex;gap:8px;align-items:center">
                <input type="hidden" name="id" value="${esc(o.id)}" />
                <input type="hidden" name="status_filter" value="${esc(active)}" />
                <span class="pill" style="background:#f3f4f6;color:#131921">${esc(o.total_items)} قطعة</span>
            ...
            <details>
              ...
              <form method="post" action="${BASE}/orders/delete" style="margin-top:10px" onsubmit="return confirm('حذف هذا الطلب نهائياً؟')">
                <input type="hidden" name="id" value="${esc(o.id)}" />
                <input type="hidden" name="status_filter" value="${esc(active)}" />
  ...
  return layout("orders", `<h1>الطلبات</h1><div class="tabs">${tabsHtml}</div>${ordersHtml}`);
}

async function pricingPage(saved: boolean): Promise<string> {
  const sb = getAdmin();
  const { data } = await sb.from("settings").select("key, value, description").order("key");
  // deno-lint-ignore no-explicit-any
  ...
  const groups = [
    ...
    { title: "أسعار الصرف والشحن (الصين)", keys: ["cny_to_usd", "usd_to_iqd_china", "china_shipping_per_kg"] },
    {
  ...
  return layout(
    ...
    <form method="post" action="${BASE}/pricing">
      ${groupsHtml}
...
async function usersPage(): Promise<string> {
  const sb = getAdmin();
  let users: Array<{ id: string; phone: string; created_at: string; last_sign_in_at: string | null }> = [];
  try {
    const { data } = await sb.auth.admin.listUsers({ page: 1, perPage: 1000 });
    // deno-lint-ignore no-explicit-any
    users = (data?.users ?? []).map((u: any) => ({
      id: u.id,
      phone: u.user_metadata?.phone ?? u.phone ?? (u.email ? String(u.email).replace(/@.*/, "") : "—"),
...
serve(async (req: Request) => {
  ...
  const idx = url.pathname.indexOf("/admin");
  let path = idx >= 0 ? url.pathname.slice(idx + "/admin".length) : url.pathname;
  if (path === "") path = "/";

  const cookieAttrs = `Path=${BASE}; HttpOnly; SameSite=Lax${secure ? "; Secure" : ""}`;

  // Login (GET)
  if (path === "/login" && req.method === "GET") {
    if (!adminPassword()) return html(notConfiguredPage());
    if (getCookie(req, COOKIE) === adminPassword()) return redirect(`${BASE}`);
  ...
  if (path === "/login" && req.method === "POST") {
    ...
    if (!password || password !== adminPassword()) {
      return html(loginPage("كلمة المرور غير صحيحة"), 401);
    }
    return redirect(`${BASE}`, {
      "set-cookie": `${COOKIE}=${encodeURIComponent(password)}; ${cookieAttrs}; Max-Age=${60 * 60 * 24 * 30}`,
    });
  }
  ...
  if (!adminPassword()) return html(notConfiguredPage());
  if (getCookie(req, COOKIE) !== adminPassword()) {
    return redirect(`${BASE}/login`);
  }
  ...
  try {
    ...
    if (path === "/orders/status" && req.method === "POST") {
      ...
      const filter = String(form.get("status_filter") ?? "all");
      if (id && STATUSES.some((s) => s.value === status)) {
        await getAdmin().from("orders").update({ status }).eq("id", id);
      }
      return redirect(`${BASE}/orders${filter === "all" ? "" : `?status=${filter}`}`);
    }

    // Orders: delete
    if (path === "/orders/delete" && req.method === "POST") {
      const form = await req.formData();
      const id = String(form.get("id") ?? "");
      const filter = String(form.get("status_filter") ?? "all");
      if (id) await getAdmin().from("orders").delete().eq("id", id);
      return redirect(`${BASE}/orders${filter === "all" ? "" : `?status=${filter}`}`);
    }
    ...
    if (path === "/pricing" && req.method === "POST") {
      ...
      for (const [key, raw] of form.entries()) {
        ...
        if (!isNaN(value)) rows.push({ key, value, updated_at: new Date().toISOString() });
      }
      if (rows.length) {
        await getAdmin().from("settings").upsert(rows, { onConflict: "key" });
      }
      return redirect(`${BASE}/pricing?saved=1`);
    }

    // GET pages
    if (path === "/" || path === "") return html(await overviewPage());
    if (path === "/orders") return html(await ordersPage(url.searchParams.get("status") ?? "all"));
    if (path === "/pricing") return html(await pricingPage(url.searchParams.get("saved") === "1"));
    if (path === "/users") return html(await usersPage());

    return html(layout("overview", `<h1>404</h1><div class="empty">الصفحة غير موجودة</div>`), 404);
  } catch (e) {
    return html(
      layout("overview", `<h1>خطأ</h1><div class="empty">${esc(e instanceof Error ? e.message : String(e))}</div>`),
      500
    );
  }
});
